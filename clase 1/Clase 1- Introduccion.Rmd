...
---
### Conocimientos (teóricos) previos del alumno:

Materia: Fundamentos de Informática

    - Python: tipos de datos, funciones (Librería Pandas)
    - Github
    - Análisis y manejo de base de datos
    - Regresiones

![https://cran.r-project.org/](Rlogo.png){width=500}
...
- Comunidad web muy grande para realizar preguntas y despejar dudas. Por ejemplo, en el caso de Buenos Aires contamos con [RenBaires(meetup)](https://www.meetup.com/es-ES/renbaires/) [RenBaires(Slack)](https://renbaires.github.io/slack) y [R-Ladies Buenos Aires](https://www.meetup.com/es-ES/rladies-buenos-aires/).
...
![https://www.rstudio.com/](RStudiologo.png){width=300}
...
- El programa permite tambien escribir en otros lenguajes como Python, C++, SQL, entre otros.
...
Los __Objetos/Elementos__ constituyen la categoría esencial del R. De hecho, todo en R es un objeto, y se almacena con un nombre específico que **no debe poseer espacios ni empezar con un numero**. Un número, un vector, una función, la progresión de letras del abecedario, una base de datos, un gráfico, constituyen para R objetos de distinto tipo. Los objetos que vamos creando a medida que trabajamos pueden visualizarse en el panel derecho superior de la pantalla (el _Environment_). 
...
```{r}
# -------------R-----------------
A <- 1 # o A = 1 en R se puede hacer de ambas formas
```
...
hasta este punto no hay diferencia con python
```{python}
# -------------python-----------------
...
```{r}
paste("Une", "estas", 4, "palabras", sep = " ")
...
__Character__
```{r}
A <-  paste('Esto', 'es', 'una', 'concatenación', 'de', 'caracteres', sep = " ")
A
class(A)
```
...
__Factor__
```{r}
A <- factor("Esto es un factor, con niveles fijos")
class(A)
```
...
También se puede crear como la combinación de N vectores de igual tamaÃÂ±o. Por ejemplo, tomamos algunos valores del [Indice trimestral de inflación en EEUU](https://fred.stlouisfed.org/series/CPIAUCSL#0) y [Indice anualizado de salarios medios por hora de todos los trabajadores,sector privado, en EEUU](https://fred.stlouisfed.org/series/CES0500000003#0)

```{r}
INDICE  <- c(	100.0, 100.0,
              101.2, 104.9,
              106.0, 109.3)
...
¿qué pasa si hacemos ```Datos$FECHA[3,2]``` ?
...
Acorde a lo visto anteriormente, el acceso a los __dataframes__ mediante  ```[ ]``` puede utilizarse para realizar filtros sobre la base, especificando una condición para las filas. Por ejemplo, puedo utilizar los  ```[ ]``` para conservar del __dataframe__ ```Datos ```  unicamente los registros con fecha de Enero 2020:

```{r error=TRUE}
Datos[Datos$FECHA=="2020",]
```

La lógica del paso anterior sería: Accedo al dataframe ```Datos```, pidiendo únicamente conservar las filas (por eso la condición se ubica a la _izquierda_ de la  ```, ```) que cumplan el requisito de pertenecer a la categoría __"2020"__ de la variable __FECHA__.    

Aún más, podría aplicar el filtro y al mismo tiempo identificar una variable de interés para luego realizar un cálculo sobre aquella. Por ejemplo, podría calcular la media de los índices en el periódo 2020. 

```{r error=TRUE}
###Por separado
Indices_2020 <- Datos$INDICE[Datos$FECHA=="2020"]
Indices_2020
```
...
```{r error=TRUE}
### Todo junto
mean(Datos$INDICE[Datos$FECHA=="2020"])
```

La lógica de esta sintaxis sería: "Me quedo con la variable __INDICE__, cuando la variable FECHA sea igual a __"2020"__, luego calculo la media de dichos valores".
...
![logo Rpoject](Rproject.png){width=500}
